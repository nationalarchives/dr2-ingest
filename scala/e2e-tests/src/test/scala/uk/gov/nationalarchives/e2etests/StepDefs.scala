...
import uk.gov.nationalarchives.e2etests.StepDefs.{Config, *}
...
class StepDefs[F[_]: Async](messageUuidCell: AtomicCell[F, List[SqsMessage]]) {
  ...
  val ingestComplete = "preserve.digital.asset.ingest.complete"
  val ingestUpdate = "preserve.digital.asset.ingest.update"
  ...
  def createBatch(ref: Ref[F, List[UUID]]): F[Unit] = ref.get.flatMap { ids =>
    val groupId = s"TST_${UUID.randomUUID}"
  ...
  def createMetadataJson(id: UUID, checksum: String, invalidMetadata: Boolean): Array[Byte] =
    def generateValue[T](ifPresent: T) = if invalidMetadata && Random.nextInt(100) > 50 then None else ifPresent.some
    val series = if invalidMetadata then None else Some("TEST 123")
    TDRMetadata(
      series,
      generateValue(id),
      None,
      ...
      s"TST-${LocalDate.now.getYear}-${makeReference(4)}",
      s"$id.txt",
      checksum,
      s"Z${makeReference(5)}"
  ...
  def createFiles(number: Int, emptyChecksum: Boolean = false, invalidMetadata: Boolean = false, invalidChecksum: Boolean = false)(using ref: Ref[F, List[UUID]]): F[Unit] = {
    ...
    for {
      ...
      _ <- updateGeneratedUuids(ids)
    } yield ()
  }

  def waitForIngestCompleteMessages(ref: Ref[F, List[UUID]]): F[Unit] =
    pollForIngestMessages(ingestComplete, ccDiskStatus, ref)

  def waitForIngestErrorMessages(ref: Ref[F, List[UUID]]): F[Unit] =
    pollForIngestMessages(ingestUpdate, assetErrorStatus, ref)
  ...
  def pollForIngestMessages(messageType: String, messageStatus: String, ref: Ref[F, List[UUID]]): F[Unit] =
    ...
    Logger[F].info(s"Polling for $messageType messages") >>
      Clock[F].timeout(pollForMessages[NotificationsOutputMessage](filterByMessageTypeStatus(messageType, messageStatus), ref), 60.minutes)
  ...
  def pollForTDRValidationMessages(ref: Ref[F, List[UUID]]): F[Unit] =
    ...
    Logger[F].info(s"Polling for TDR validation messages") >>
      Clock[F].timeout(pollForMessages[SqsInputMessage](filterByInputMessage, ref), 60.minutes)
  ...
  def pollForMessages[T <: SqsMessage](filter: (SqsMessage, List[UUID]) => Boolean, ref: Ref[F, List[UUID]])(using decoder: Decoder[T], queueUrl: String): F[Unit] = {
    for {
      ...
      res <- if newInputUuids.isEmpty then
        Async[F].unit
      else if messages.isEmpty then
        Async[F].sleep(60.seconds) >> pollForMessages(filter, ref)
      else
        pollForMessages(filter, ref)
    } yield ()
  }
}

object StepDefs {
  def apply[F[_]: Async](messageUuidCell: AtomicCell[F, List[SqsMessage]]) = new StepDefs(messageUuidCell)


  case class Config(inputBucket: String, ingestSqsQueue: String, notificationsSqsQueue: String, lockTableName: String, preingestSfnArn: String, copyFilesDlq: String)derives ConfigReader

...
import uk.gov.nationalarchives.dynamoformatters.DynamoFormatters.{given_Schema_AssetDynamoTable, *}
...
//    ),
    (
      ...
      "'skipIngest': not of type: 'Boolean' was '\"M\": { \"skipIngest\": { \"S\": \"true\" } }'",
      Asset
    ),
//    (
...
//        case ContentFolder => contentFolderTableFormat
        case Asset         => given_Schema_AssetDynamoTable
//        case File          => fileTableFormat

      val res = dynamoTableFormat.read(DynamoValue(attributeValue))
      res.isLeft should be(true)
      res.left.value.message should equal(expectedErrors)
    }
  }

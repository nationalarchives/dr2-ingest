...
object Main extends IOApp {
  ...
  private val since: ZonedDateTime = ZonedDateTime.parse("2025-01-01T00:00:09.000000000Z[Europe/London]")

  given Logger[IO] = Slf4jLogger.getLogger[IO]


  override def run(args: List[String]): IO[ExitCode] =
    Fs2Client.entityClient("https://tna.preservica.com", "intg-preservica-api-login-details-A1YMZ1t9Qb").flatMap { client =>
      ...
      def update(start: Int): IO[Unit] = {
        for {
          ...
          _ <- Logger[IO].info(s"Found ${updatedPage.length} entities")

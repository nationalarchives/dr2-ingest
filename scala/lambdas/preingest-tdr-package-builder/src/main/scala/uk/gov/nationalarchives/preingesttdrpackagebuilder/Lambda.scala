  ...
  override def handler: (Input, Config, Dependencies) => IO[Output] = (input, config, dependencies) => {
    ...
    def createAsset(
      ...
      AssetMetadataObject(
        ...
        potentialMessageId,
        List(
          IdField("Code", if config.sourceSystem == `Parliament Migration` then packageMetadata.fileReference else s"${packageMetadata.series}/${packageMetadata.fileReference}"),

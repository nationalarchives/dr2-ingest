...
class LambdaTest extends AnyFlatSpec with ScalaCheckDrivenPropertyChecks:
  ...
  } yield TestData(series, body, date, tdrRef, fileName, fileSize, List(checksum), fileRef, groupId, batchId)
  ...
  "lambda handler" should "return an error if the s3 upload fails" in {
    ...
    val tdrMetadata = TDRMetadata("", UUID.randomUUID, None, None, Option(""), "2024-10-04 10:00:00", "", "test.txt", checksum(""), "")

...
class LambdaRunnerTest extends AnyFlatSpec with EitherValues {
  def inputString: String = Input("input").asJson.printWith(Printer.noSpaces)
  ...
  "handleRequest" should "return the correct output" in {
    ...
    val config = Config("configValue", "dependencyReturnValue")
    val dependencies = Dependencies(TestDependency(config.dependencyReturnValue))
    val appContext = AppContext[Config, Dependencies](config, dependencies)
    val output = lambdaRunner.runLambda(inputString, appContext)
...
//    lambdaRunner.runLambda(inputString, os, null)
//

...
class LambdaTest extends AnyFlatSpec with MockitoSugar with BeforeAndAfterEach {
  ...
  "handler" should "write the correct values to dynamo" in {
    ...
    checkDynamoItems(
      tableRequestItems,
      DynamoTable("TEST", UUID.fromString(uuids.head), "", "A", ArchiveFolder, "Test Title A", "TestDescriptionA with 0", Some("A"), 1, fixedTime)
    )
    checkDynamoItems(
      tableRequestItems,
      DynamoTable("TEST", UUID.fromString(uuids.tail.head), uuids.head, "A 1", ArchiveFolder, "Test Title A 1", "TestDescriptionA 1 with 0", Some("A 1"), 1, fixedTime)
    )
    checkDynamoItems(
      tableRequestItems,
      DynamoTable(

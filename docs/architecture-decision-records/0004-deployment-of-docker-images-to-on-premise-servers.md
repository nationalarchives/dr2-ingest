...
It is possible to install the Code Deploy agent on an on-premises server. This allows you to run Code Deploy builds on that server. Code Deploy can be linked into ECR which would allow us to deploy from within the AWS infrastucture without polling on the server. 

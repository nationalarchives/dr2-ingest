...
## Context
As we [migrate from DRI](/python/dri-migration/README.md), we will be uploading files to Preservica (DR2) from Kew. 
Custodial Copy (CC), in its current state, downloads the file from Preservica and store it in Kew which, for the files we
...
When CC is about to download a file from Preservica, it will check this local cache to see if the exact file 
is already available there, and sources it locally if so, otherwise, it will source it from Preservica; files could be added to,
or removed from, the cache by other parties/systems. We would want to begin the upload and ingest process from this cache
location also.
...
- In CC, for a given file, right before it downloads the file from Preservica, it will use the file id in order to call the
  SQLite database table and retrieve the file's local file path.
- It will use the file path to download the file to the location where Preservica downloads go to and then continue with
  the other processes.

module "dr2_ingest_start_workflow_lambda" {
  ...
  function_name   = var.lambda_names.start_workflow
  s3_bucket       = local.code_deploy_bucket
  s3_key          = replace("${var.deploy_version}/${var.lambda_names.start_workflow}", "${local.environment}-dr2-", "")
  handler         = "uk.gov.nationalarchives.ingeststartworkflow.Lambda::handleRequest"
  timeout_seconds = local.java_timeout_seconds
  policies = {
    "${var.lambda_names.start_workflow}-policy" = templatefile("./templates/iam_policy/ingest_start_workflow_policy.json.tpl", {
      account_id                 = data.aws_caller_identity.current.account_id
      lambda_name                = var.lambda_names.start_workflow
  ...
  tags = {
    Name = var.lambda_names.start_workflow
  }
}

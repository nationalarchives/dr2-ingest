...
module "dr2_ingest_flow_control_lambda" {
  ...
  s3_key          = replace("${var.deploy_version}/${local.ingest_flow_control_lambda_name}", "${local.environment}-dr2-", "")
  ...
  policies = {
    "${local.ingest_flow_control_lambda_name}-policy" = templatefile("./templates/iam_policy/ingest_flow_control_policy.json.tpl", {
      ...
      dynamo_db_queue_table_arn  = var.ingest_queue_table_arn
      ssm_parameter_arn          = var.flow_control_config.arn
  ...
  plaintext_env_vars = {
    ...
    CONFIG_PARAM_NAME = var.flow_control_config.name
  ...
  vpc_config = {
    ...
    security_group_ids = [var.outbound_https_access_only_id]
  }
}
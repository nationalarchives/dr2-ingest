...
module "tdr_preingest" {
  ...
  ingest_lock_table_arn               = var.ingest_lock_table_arn
  ingest_lock_table_group_id_gsi_name = local.ingest_lock_table_group_id_gsi_name
  ingest_raw_cache_bucket_name        = var.ingest_raw_cache_bucket_name
  ingest_step_function_name           = local.ingest_step_function_name
  sns_topic_arn                       = "arn:aws:sns:eu-west-2:${module.tdr_config.account_numbers[local.default_environment]}:tdr-external-notifications-${local.environment}"
  source_name                         = "tdr"
  bucket_kms_arn                      = module.tdr_config.terraform_config["${local.default_environment}_s3_export_bucket_kms_key_arn"]

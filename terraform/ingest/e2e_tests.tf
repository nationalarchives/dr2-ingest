...
module "dr2_e2e_tests_policy" {
  ...
  policy_string = templatefile("${path.root}/templates/iam_policy/e2e_tests_policy.json.tpl", {
    ...
    external_notifications_log_group = var.external_notifications.log_group_arn
    copy_files_from_tdr_log_group    = "arn:aws:logs:eu-west-2:${data.aws_caller_identity.current.account_id}:log-group:/aws/lambda/${var.preingest.tdr.importer_lambda_name}"
  })
}

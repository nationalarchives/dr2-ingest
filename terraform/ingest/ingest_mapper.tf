module "dr2_ingest_mapper_lambda" {
  ...
  function_name   = var.lambda_names.ingest_mapper
  s3_bucket       = local.code_deploy_bucket
  s3_key          = replace("${var.deploy_version}/${var.lambda_names.ingest_mapper}", "${local.environment}-dr2-", "")
  handler         = "uk.gov.nationalarchives.ingestmapper.Lambda::handleRequest"
  timeout_seconds = local.java_timeout_seconds
  policies = {
    "${var.lambda_names.ingest_mapper}-policy" = templatefile("./templates/iam_policy/ingest_mapper_policy.json.tpl", {
      raw_cache_bucket_name    = var.ingest_raw_cache_bucket_name
      ingest_state_bucket_name = local.ingest_state_bucket_name
      account_id               = data.aws_caller_identity.current.account_id
      lambda_name              = var.lambda_names.ingest_mapper
  ...
  tags = {
    Name = var.lambda_names.ingest_mapper
  }
}

...
module "dr2_ingest_failure_notifications_lambda" {
  ...
  policies = {
    "${local.ingest_failure_notifications_lambda_name}-policy" = templatefile("${path.root}/templates/iam_policy/failure_notifications_policy.json.tpl", {
      ...
      dynamo_db_file_table_arn = var.ingest_lock_table.arn
      gsi_name                 = var.ingest_lock_table.group_id_gsi_name
  ...
  plaintext_env_vars = {
    ...
    LOCK_DDB_TABLE_GROUPID_GSI_NAME = var.ingest_lock_table.group_id_gsi_name

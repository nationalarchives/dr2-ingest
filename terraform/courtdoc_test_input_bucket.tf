locals {
  courtdoc_test_bucket_name = "${local.environment}-dr2-ingest-parsed-court-document-test-input"
  tre_prod_event_bus        = local.tre_terraform_prod_config["da_eventbus"]
}

module "courtdoc_test_input_bucket" {
...
moved {
  from = module.dr2_ingest_parsed_court_document_event_handler_test_input_bucket
  to   = module.courtdoc_test_input_bucket
}
package uk.gov.nationalarchives.preservicaconfig
...
object FileProcessors {
  ...
  private[preservicaconfig] def processSchemas(key: String, xmlData: String): SchemaFileInfo =
    SchemaFileInfo(key.withoutFileExtension, "", key.getName, xmlData)

  private[preservicaconfig] def processTransforms(key: String, xmlData: String): TransformFileInfo = {
  ...
  private[preservicaconfig] def processIndexDefinitions(key: String, xmlData: String): IndexDefinitionInfo =
    IndexDefinitionInfo(key.withoutFileExtension, xmlData)

  private[preservicaconfig] def processMetadataTemplates(key: String, xmlData: String): MetadataTemplateInfo =
    MetadataTemplateInfo(key.withoutFileExtension, xmlData)

  private[preservicaconfig] def processFiles(
  ...
  private[preservicaconfig] def s3ObjectsFromEvent(input: SQSEvent): IO[List[S3Object]] =
